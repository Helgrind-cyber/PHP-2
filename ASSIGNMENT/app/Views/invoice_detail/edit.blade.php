...
@section('page', 'Chi tiết hóa đơn')
@section('short_desc', '*: Không được để trống')

@section('main-content')
<div class="container">
    <form action="{{getClientUrl('invoice-detail/save-edit')}}" method="post" id="createInvoiceDetail">
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="customer_name">Khách hàng<span class="text-danger">*</span></label>
                    <input type="text" id="customer_name" class="form-control" name="invoice_id" placeholder="Nhập tên khách hàng" value="{{$invoice_detail->invoice_id}}">
                    @if (isset($_GET['customer_nameerr']))
                    <div cla
        dd($invoice_detail);ss="alert alert-danger" role="alert">{{$_GET['customer_nameerr']}}</div>
                    @endif
                </div>
                <div class="form-group">
                    <label for="customer_phone_number">Số điện thoại khách hàng<span class="text-danger">*</span></label>
                    <input type="text" id="customer_phone_number" class="form-control" name="customer_phone_number" placeholder="Nhập số điện thoại khách hàng">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" id="customer_email" class="form-control" name="customer_email" placeholder="Nhập Email khách hàng">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="customer_address" class="form-control" name="customer_address" placeholder="Nhập địa chỉ khách hàng">
                    @if (isset($_GET['customer_addresserr']))
                    <div class="alert alert-danger" role="alert">{{$_GET['customer_addresserr']}}</div>
                    @endif
                </div>
                <div class="d-flex justify-content-end my-2">
                    <button type="submit" class="btn btn-outline-primary">Thêm hóa đơn</button>&nbsp;
